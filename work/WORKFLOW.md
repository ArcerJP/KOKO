...
チームでの共有先や範囲を変更する場合は、プライバシー、セキュリティ、履歴、協働方法をレビューしたうえで、ポリシーを明示的に変更します。公開KOKOへ非公開内容を強制追加してはいけません。

2026-09-23に、メインメンバーへの資料共有を別のPrivateリポジトリで行う方針が承認されました。[非公開共有](../docs/private-sharing.md)に従い、引き渡し時には非公開側への反映状況または未反映理由も確認します。PublicのKOKOへの追跡追加は許可しません。Private側のcontextは受渡し用の記録であり、実行中タスクの配置場所や状態を別に増やしません。
