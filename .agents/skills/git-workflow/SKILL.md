...
操作経路は[TOOLS.md](../../../TOOLS.md#github操作ツール)に従い、書込みには既存のGitと確認済みのGitHub連携を使用します。PR・checkの閲覧や調査は、ブラウザでも[Web操作の利用条件](../../../TOOLS.md#web操作の利用条件)の確認不要な範囲で行えます。連携の不具合・権限不足・認証アカウント不一致を、ブラウザによるPR作成・ログイン・設定変更等への自動切替えで回避しません。その方法での操作が必要なら、対象と目的を説明してユーザーへ事前確認します。
