...
- PDF、非公開の動的コンテキスト、dist等のbuild生成物は自動整形の対象外です。追跡する生成型・エラー表は生成スクリプト内でPrettierを適用し、再生成一致を検査します。
- SQLは標準Prettierの対象外です。UTF-8/LF・空白2個をEditorConfigで揃え、migration適用テストで構文を検証します。
- Node.jsとnpmは整形・文書解析に加えて共有契約の生成と検証に使用します。現在の採用構成は[プロダクト構成](architecture/product-architecture.md)を参照してください。
